...
describe('BookingController', () => {
    ...
    describe('checkIn() - controller', () => {
        const { checkIn } = require('../../src/controllers/BookingController');
        const { calculateDistance } = require('../../src/utils/geoUtils');

        it('should return 400 if driver location missing', async () => {
            const req = mockReq({
                params: { id: 'booking-uuid' },
                body: {}, // no driverLocation
                user: { id: 'driver-uuid', role: 'driver' },
            });
            const res = mockRes();
        ...
        it('should return 404 if booking not found', async () => {
            const req = mockReq({
                params: { id: 'nonexistent-uuid' },
                body: { driverLocation: { lat: 6.9271, lng: 79.8612 } },
                user: { id: 'driver-uuid', role: 'driver' },
            });
            const res = mockRes();
        ...
        it('should return 403 if not drivers booking', async () => {
            const req = mockReq({
                params: { id: 'booking-uuid' },
                body: { driverLocation: { lat: 6.9271, lng: 79.8612 } },
                user: { id: 'other-driver', role: 'driver' },
            });
            const res = mockRes();
            ...
            Booking.findById.mockResolvedValue({
                ...
                spot_latitude: '6.9271',
                spot_longitude: '79.8612',
            });
        ...
        it('should return 400 if booking not confirmed', async () => {
            ...
            Booking.findById.mockResolvedValue({
                id: 'booking-uuid',
                driver_id: 'driver-uuid',
                booking_status: 'pending', // not confirmed
                spot_latitude: '6.9271',
                spot_longitude: '79.8612',
            });
            ...
            expect(res.json).toHaveBeenCalledWith(
                expect.objectContaining({
                    error: expect.stringContaining("Must be 'confirmed'"),
                })
            );
        });

        it('should return 400 if driver too far from spot', async () => {
            const req = mockReq({
                params: { id: 'booking-uuid' },
                body: { driverLocation: { lat: 7.0000, lng: 80.0000 } }, // far away
                user: { id: 'driver-uuid', role: 'driver' },
            });
            const res = mockRes();
            ...
            calculateDistance.mockReturnValue(500); // 500 meters away
            ...
            expect(res.json).toHaveBeenCalledWith(
                expect.objectContaining({
                    error: expect.stringContaining('Too far'),
                })
            );
        });

        it('should check in successfully when close to spot', async () => {
            const req = mockReq({
                params: { id: 'booking-uuid' },
                body: { driverLocation: { lat: 6.9271, lng: 79.8612 } },
                user: { id: 'driver-uuid', role: 'driver' },
            ...
            calculateDistance.mockReturnValue(10); // 10 meters - close enough

            Booking.checkIn.mockResolvedValue({
                id: 'booking-uuid',
    ...
    describe('checkOut() - controller', () => {
        const { checkOut } = require('../../src/controllers/BookingController');
        ...
        it('should return 403 if not drivers booking', async () => {
            ...
            Booking.findById.mockResolvedValue({
                id: 'booking-uuid',
                driver_id: 'driver-uuid', // different
                booking_status: 'active',
            });
        ...
        it('should return 400 if booking not active', async () => {
            ...
            Booking.findById.mockResolvedValue({
                id: 'booking-uuid',
                driver_id: 'driver-uuid',
                booking_status: 'confirmed', // not active
            });
            ...
            expect(res.json).toHaveBeenCalledWith(
                expect.objectContaining({
                    error: expect.stringContaining("Must be 'active'"),
                })
            );
        });

        it('should checkout successfully with no overtime', async () => {
            const req = mockReq({
                params: { id: 'booking-uuid' },
                user: { id: 'driver-uuid', role: 'driver' },
            });
        ...
        it('should checkout with overtime message', async () => {
            const req = mockReq({
                params: { id: 'booking-uuid' },
                user: { id: 'driver-uuid', role: 'driver' },
            });
            ...
            Booking.checkOut.mockResolvedValue({
                ...
                overtime_hours: '1.00', // has overtime
                expected_price_xrp: '2.000000',
